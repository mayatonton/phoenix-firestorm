...
3. **MDI emit は forEachSource の上流の明示入口のみ**: textured = pushBatches(lldrawpool.cpp:1199-1216・isCameraMdiPass+heap gate)/ untextured = pushUntexturedBatches(:1231-1245・影 opaque MV の経路)/ materials pool 入口(lldrawpoolmaterials.cpp:149-166・E 方式)/ rigged = pushRiggedBatchesIndirect(lldrawpool.cpp:1432・renderMap walk-accumulate)。新 MDI 入口の追加 = 本 doc 改訂事項。
