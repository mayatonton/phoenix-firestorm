...
**仕様**: `doc/spec_binaural_venue_reverb.md`
...
**ファイル**:
- `doc/spec_binaural_venue_reverb.md` (新規、初版 → 配信者主導モデル全面書き換え → URL pre-resolve P10 組込み)
...
**ファイル**:
- `doc/r11/gen_test_material.sh` (sox + espeak-ng + oggenc)
- `doc/r11/fetch_venue_irs.sh` (P7c-B 由来、IR DL/変換 helper)
...
| 項目 | 参照先 |
|---|---|
| 設計判断 (旧 r5 NG1 反転 / 旧 Steam Audio 案棄却) | `doc/spec_binaural_venue_reverb.md` §1 / §2 |
| リスク R1〜R8 (内容 + 対策方針) | `doc/spec_binaural_venue_reverb.md` §8 |
| 受入条件表 (実測値含む) | `doc/spec_binaural_venue_reverb.md` §5.5 |
| 実装サマリ + DSP 詳細 | `doc/spec_binaural_venue_reverb.md` §13.1 / §4.3 / §4.4 |
| §5.4 安定性指標まとめ | `doc/spec_binaural_venue_reverb.md` §13.2 |
| codec 別 end-to-end 検証状況 | `doc/spec_binaural_venue_reverb.md` §13.4 / §5.4 |
| 既知の運用上の制約 (CPU note 含む) | `doc/spec_binaural_venue_reverb.md` §13.5 |
| r12+ への持ち越し | `doc/spec_binaural_venue_reverb.md` §13.6 / §9 |
| GitHub release 告知文案 | `docs/ayastorm-r11-release-note.md` |
| IR ライセンス / attribution | `indra/newview/app_settings/venue_ir/CREDITS.md` |
| 検証材料生成 | `doc/r11/gen_test_material.sh` / `doc/r11/fetch_venue_irs.sh` |
