...
Feature details live in the user-facing guide (`doc/3dstream-tag-guide.{ja,en,zh}.md`) and the spec / roadmap docs. This note is link-only + diff highlights.
...
- **Syntax**: bare `[ayastorm:occlude]` uses defaults (`direct:0.7 reverb:0.5`); `[ayastorm:occlude{direct:0.9}{reverb:0.7}]` overrides per prim. Details → [tag-guide §16](../doc/3dstream-tag-guide.en.md#16-static-occlusion-ayastormocclude-r13) / spec `doc/spec_obb_occlusion.md` / implementation record `docs/ayastorm-r13-occlusion.md`
...
- **Exact-shape decision**: sound passes through Path Cut openings / Hollow interiors and is muffled only by the wall body itself; mesh prims use their precise geometry. A two-stage test — bounding-OBB pre-cull (~95% reject) → Möller-Trumbore triangle raycast — keeps CPU low while preserving full-shape accuracy (details → [tag-guide §16.2](../doc/3dstream-tag-guide.en.md#162-behaviour-model)).
...
- User-facing guide: `doc/3dstream-tag-guide.ja.md` / `.en.md` / `.zh.md` (§16 added — full static occlusion section)
