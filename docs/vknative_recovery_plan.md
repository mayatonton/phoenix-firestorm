...
| **E3** | **スコープ張り替え(2026-07-19 夜・E3-0 計器 = e3 欄で実測裁定)**: 本丸 = **①palette index 化(1 avatar 1 回/frame・shadow per-map 再 upload 根絶)+ ②shadow rigged 発行の bucket 化**。camera rigged 発行は 0.03ms/f と実測 = 非標的に降格(camera rigged の重さは alp set / aChar 側)。北極星(100 体会場)線形外挿で shadow rig ~2ms/f 級・pal ~0.7ms/f 級 | e3 計器実測(crowd settled): rig shadow 0.73ms/f(records 640/f)・pal 0.25ms/f(382 回/f・理論下限 ~40 回/f = 90% 削減余地)・rig camera 0.03ms/f | 視覚同一 + validation 0 + e3 欄 pal/rig shadow 崩落 | E2 |
