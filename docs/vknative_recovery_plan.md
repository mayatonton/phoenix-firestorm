...
| 段 | 内容 | 実測根拠 | gate | 依存 |
|---|---|---|---|---|
| **E1** | **alpha の sorted-indirect 化**: CPU ソート維持(順序不変条件)・emission を「ソート順に indirect command を ring へ線形書き」に置換。per-draw set/pipeline/PC 鎖を pass 頭 1 回へ | alpha = camera pool 最大 2.8〜9.6ms・2.2k draws/f | 視覚同一(alpha 順序!)+ validation 0 + fam alpha の崩落 | M5a 資産(済) |
| **E2** | **materials 系の bindless 拡大 + 13-pass MDI 合流**: normal/spec 等を heap slot 化し DrawData/record 表を拡張 → camera 側 MDI 対象を 13 pass 全部へ | materials 1.9〜3.1ms・1.6k draws/f | 同上 + fam materials 崩落 | E1 の後推奨(ring/表の変更が重なる) |
...
| **E4** | **lighting 発行鎖のトレースと迂回**: renderDeferredLighting の CPU 4.5〜9ms の内訳(fullscreen/local light が universal set/populate を通る疑い)→ 発行形の固定化 | lighting CPU 4.5〜9ms | 同上 | なし(トレースは並列可) |
