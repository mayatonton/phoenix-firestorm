...
AYAstorm r31.2 では、通常の AO セット削除を「非表示」に置き換え、データを消さないように直しました。ただし Hidden 管理画面内の **「選択を削除」** (`Delete selected`) は、確認後に実 inventory folder を完全削除する整理用操作です。
...
注意: Hidden 管理画面の **「選択を削除」** (`Delete selected`) は例外です。これは選択した非表示セットの実 inventory folder を完全削除する操作なので、不要な AO を整理したい場合だけ使ってください。
...
3. 非表示にしたセットの一覧が出る
4. 個別に戻したい → セットを選択 → **「選択を再表示」** (`Restore selected`)
5. 全部戻したい → **「すべて再表示」** (`Restore all`)

通常の「非表示」ではデータ自体は触られていないので、Firestorm 本家など他の viewer でログインしても、これらのセットは残ったままです。

### 「選択を削除」について

Hidden 管理画面には **「選択を削除」** (`Delete selected`) もあります。これは通常の「非表示」ではありません。

- 選択した非表示 AO セットの実 inventory folder を `#Firestorm/#AO` から完全削除します
- Firestorm 本家や他の Firestorm 派生 viewer からログインしても、その AO セットは消えます
- AYAstorm 側から元に戻すことはできません
- backup / notecard がない場合、削除した AO データは復元できません

単に AO ウィンドウから見えなくしたいだけなら **「選択を削除」は使わず**、通常の Hide と Restore を使ってください。
