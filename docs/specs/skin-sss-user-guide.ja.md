...
| アバター Mesh body / head | 対象 Mesh の asset UUID が whitelist (§4) に登録されているもの。出荷時 default whitelist に主要 Mesh body / head は含まれています。 |
...
出荷時の default whitelist には主要 Mesh body / head 製品が含まれているため、多くのユーザーは登録作業なしで SSS が機能します。
...
4. **Enabled SSS** を ON。
5. アバターの顔を見る。プラスチックっぽいままなら Mesh head が default whitelist に無い可能性 — 頭を右クリック → **Add to SSS whitelist**。
6. もっと柔らかくしたい場合は **Strength** を 0.7 へ。肌が「死んで」見えるようなら **Glow gain** を 0.3 程度に。
...
whitelist の default は主要 Mesh body / head UUID のキュレーション済リストなので、リセットすると独自追加分は上書き消去されます。承知のうえで使ってください。
...
| 顔だけ柔らかく、手足はプラスチックのまま | 手足が別 Mesh asset で whitelist 未登録 | 該当パーツを右クリック → Add to SSS whitelist |
