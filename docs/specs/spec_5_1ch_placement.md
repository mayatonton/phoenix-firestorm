...
> **前提**: `docs/specs/spec_5_1ch_source.md` (r9 5.1ch ソース受入) — 本書はその §9.1 / §12.6 を独立化したもの
> **前提**: `docs/specs/spec_distributed_stereo.md` (r8 分散記述ステレオ仕様)
...
スクリプトは `docs/archive/r10/gen_test_material.sh` として repo に置く (再現性保証)。生成済みの ogg は repo に入れず、AYA Icecast にホスト。
...
| **P8** | テスト材料生成スクリプト + AYA Icecast hosting | `docs/archive/r10/gen_test_material.sh`, `test_routing_5_1.ogg` URL 確定 | §5 |
...
| P8 | `c90b67e649` | 5.1ch routing 検証材料生成スクリプト (`docs/archive/r10/gen_test_material.sh`) |
...
→ **詳細は `docs/specs/spec_binaural_venue_reverb.md` (r11 仕様書) を参照**
