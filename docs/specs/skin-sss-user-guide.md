...
- Hard speculars on the skin are gently softened (no more "plastic" highlights)
...
| Avatar mesh body / head | Any Mesh asset whose UUID is registered in the whitelist (§4). The shipped default whitelist covers the most common Mesh bodies and heads. |
...
The shipped default whitelist already covers the most common Mesh body and head products, so for most users SSS works out of the box without any registration.
...
4. Check **Enabled SSS**.
5. Look at your avatar's face. If the skin still looks plastic, your Mesh head is probably not in the default whitelist — right-click the head → **Add to SSS whitelist**.
6. If you want a softer look, raise **Strength** toward 0.7. If the skin starts looking "dead", raise **Glow gain** to ~0.3.
...
The whitelist defaults are a curated list of the most common Mesh body / head UUIDs; resetting will overwrite any custom additions, so use it knowingly.
...
| Some parts of the body (hands / feet) stay plastic while the face is soft | Hands / feet are a separate Mesh asset and not in the whitelist | Right-click them → Add to SSS whitelist |
