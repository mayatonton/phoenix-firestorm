...
- 仕様: `docs/specs/spec_stream3d_decode_thread.md`
...
- 仕様: `docs/specs/spec_distributed_stereo.md`
...
- 仕様: `docs/specs/spec_5_1ch_source.md`
...
- 仕様: `docs/specs/spec_5_1ch_placement.md`
...
- 仕様: `docs/specs/spec_binaural_venue_reverb.md` (PR #31, #36 マージ済、本リリースで実装着手)
...
- **r5 NG1 (HRTF/binaural 不要) を反転**: r10 で 5〜30m 会場モデル + 6 spk 同時定位に拡張された結果、ITD なし / air abs なし / 会場残響なしの薄さが体感的に目立つようになったため (詳細は `docs/specs/spec_binaural_venue_reverb.md` §2.3)
...
- 仕様: `docs/specs/spec_obb_occlusion.md` (final scope 確定 2026-05-11)
...
- **設計判断 — 旧 r13+ basket からの再定義**: 「3D stream を実装してプリムから音を出せるようになったが、音を遮る/反射する prim を作れないか検討したい」(AYA、2026-05-10) という提案が起点。OBB 単独で出荷、Steam Audio (回折/反射/共鳴) は r14+ に保留することで、SL viewer 史上初の空間音響遮蔽機能を viewer 内 DSP 完結で実現。配布負債ゼロ + 既存配置全部に効く + r14+ Steam Audio の geometry 登録基盤として再利用可能、の三拍子 (詳細は `docs/specs/spec_obb_occlusion.md` §1 / §2.2)
...
詳細は `docs/specs/spec_binaural_venue_reverb.md` §11 (実装フェーズ) / §12 (工数見積)。
...
詳細は `docs/specs/spec_stereo_upmix.md` §8 / `docs/ayastorm-r12-stereo-upmix.md` §2 (実装フェーズ)。
...
詳細は `docs/specs/spec_obb_occlusion.md` §8 / `docs/ayastorm-r13-occlusion.md` §5 (実装ログ canonical)。
...
- P10 検証 scene 構築手順 (`docs/archive/r13/build_test_scene.md`、推奨セット 石壁 0.9/0.7 / 木壁 0.6/0.4 / ガラス 0.3/0.2 の聴感確認用): 0.5 日
