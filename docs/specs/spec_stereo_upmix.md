...
> **前提**: `docs/specs/spec_5_1ch_placement.md` (r10 5.1ch placement 仕様)
> **前提**: `docs/specs/spec_5_1ch_source.md` (r9 5.1ch ソース受入)
> **前提**: `docs/specs/spec_binaural_venue_reverb.md` (r11 lite-HRTF + venue convolution reverb、配信者主導モデル)
...
- `docs/specs/spec_5_1ch_placement.md` (r10) で確立した per-channel placement の経路は本書でも維持。upmix DSP は **6ch 出力を生成して既存 r10 placement の入力にそのまま流す** 構造で挿入する (= placement 側に変更ゼロ)
- `docs/specs/spec_binaural_venue_reverb.md` (r11) の lite-HRTF / venue reverb DSP chain との順序は §4.2.3 で明記
...
**背景**: 当初の本書 (旧 §4.2.1) では「FMOD DSP として upmix を挿入する A 案 (group input) / B 案 (per-binding)」を候補としたが、P0 で実コードを読んだ結果、両案ともアーキテクチャ的に不適合と判明 (詳細: `docs/archive/r12/dsp_insertion_survey.md`)。理由は以下:
...
検証材料生成は `docs/archive/r12/gen_upmix_test_material.sh` (新規) で `ffmpeg` ベースに作成。r11 の `gen_test_material.sh` の流儀を踏襲。
...
| ~~R6~~ | ~~upmix DSP 挿入箇所 (A 案 vs B 案) で **想定外の DSP chain 不整合** が発生~~ | **解消 (2026-05-07)**: P0 調査で C 案 (`SpeakerCallback::OpKind` 拡張) として確定、A/B 案は廃案 (詳細: `docs/archive/r12/dsp_insertion_survey.md`)。Bs775 dispatch の並行構造で実装するため DSP chain 不整合の余地なし |
...
- **P0**: 仕様確定 + roadmap doc 同時更新 + 実装箇所調査 → **C 案 (`OpKind::Upmix` 拡張) として確定** (`docs/archive/r12/dsp_insertion_survey.md`)
