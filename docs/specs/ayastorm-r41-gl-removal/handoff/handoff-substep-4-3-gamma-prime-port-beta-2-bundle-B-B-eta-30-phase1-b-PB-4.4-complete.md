# Handoff: sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-30 Phase 1.B PB-4.4 完了

**作成日**: 2026-06-04
**本 session 物理 commit**:
- `71576ff1e4` (= PB-4.4 uniform2f integer index 経路 Vulkan path 分岐、+15 line)

**次 session 着手**: **AYA 判断不要 自走** → **PB-4.5 着手** = `uniform3f(U32 index, GLfloat x, GLfloat y, GLfloat z)` llglslshader.cpp:2354 に Vulkan path 分岐追加 (= scalar 引数 → 一時 array `GLfloat tmp[3] = {x, y, z};` + `forwardToUboUpload(loc, tmp, sizeof(tmp))`)、1 method 1 sub-step strict 線形。

---

## §0 state 一行 summary

η-30 **Phase 1.B PB-4.4 完了 state** (= 直前 commit `71576ff1e4`):
- **PB-4.4 uniform2f** 完了 = llglslshader.cpp glUniform2f 直前に if (mUseUBO) { ... GLfloat tmp[2]={x,y}; forwardToUboUpload(loc, tmp, sizeof(tmp)); return; } 分岐追加、+15 line
- **自動進行廃止反映** = batch 3 緩和終了、`feedback_ubo_migration_one_at_a_time` 復帰 = 1 method 1 sub-step、本 session = PB-4.4 単独進行
- **残 sub-step**: PB-4.5〜PB-4.17 (13 method) → PB-5.1〜.13 (13 method) → PB-7 → PB-N strict 線形
...
| # | file | 読む箇所 | 目的 |
|---|---|---|---|
| 1 | 本 handoff doc (= `handoff-substep-4-3-gamma-prime-port-beta-2-bundle-B-B-eta-30-phase1-b-PB-4.4-complete.md`) | 全文 | PB-4.4 完了 state + 残 PB-4.5〜.17 順序 + PB-4.5 着手第 1 step |
| 2 | `docs/specs/ayastorm-r41-gl-removal/design/06a-cache-structure-and-setter-redirect.md` | §5.2 + §5.3 (= integer index path 分岐 code shape + 規律表) | PB-4.5〜.17 の pattern source-of-truth |
| 3 | `indra/llrender/llglslshader.cpp:2354-2380` | `uniform3f` 既存実装 + 周辺 | PB-4.5 着手前 挿入位置確定 |

### §1.2 pinpoint Read 用 reference (= 必要時のみ)

| file | 参照箇所 |
|---|---|
| `indra/llrender/llglslshader.cpp:2327-2367` | PB-4.4 (uniform2f) Vulkan path 分岐 pattern (= 直前 commit、PB-4.5 pattern source) |
...
## §2 残 sub-step strict 線形構成 (= PB-4.4 完了反映)
...
| **PB-4.5** | uniform3f(U32 index, GLfloat x, GLfloat y, GLfloat z) Vulkan path 分岐 = tmp[3]={x,y,z} 一時 array + forwardToUboUpload(loc, tmp, sizeof(tmp)) | llglslshader.cpp | 次 session 着手 |
...
**注 (= PB-4.5〜4.6 uniform3f/4f)**: scalar 引数から ptr 生成 = `GLfloat tmp[3] = {x, y, z};` / `GLfloat tmp[4] = {x, y, z, w};` 後 `tmp` を渡す (= sizeof(tmp) で N*sizeof(GLfloat) 自動算出)。

---

## §3 本 session 新規確定 設計判断

**設計判断 0 件** (= 全 default 採用済、PB-4.4 は PB-6 complete handoff §6 + PB-4.2 (uniform1f) の code shape literal そのままで適用、AYA 確認不要)。

scalar 引数 → 一時 array pattern の literal:
```cpp
GLfloat tmp[2] = {x, y};
forwardToUboUpload(loc, tmp, sizeof(tmp));
```
(= `sizeof(tmp) = 2*sizeof(GLfloat)` で N 要素を明示せずに済む = `&tmp[0]` よりも array decay 経路で書きやすい)。
...
| (2) 挿入位置 | glUniform2f 直前 (= 既存 OpenGL path より前に Vulkan path 分岐) | ✅ |
...
| (6) 残 sub-step strict 線形維持 | §2 table = PB-4.5 → ... → PB-N | ✅ |
| (7) 1 method 1 sub-step 復帰 | 自動進行廃止で `feedback_ubo_migration_one_at_a_time` 復活、batch 3 緩和終了 | ✅ |
...
## §5 引き継ぎ済 memory (= PB-4.1〜4.3 handoff 継承、追加なし)
...
- `feedback_ubo_migration_one_at_a_time` — **1 method 1 sub-step (= 自動進行廃止で復帰、batch 3 緩和終了)**
- `feedback_doubt_self_first` — build error / link fail は AYA 投げ前に Claude root cause 特定
- `feedback_no_auto_commit` — 通常 session では AYA 明示指示後 commit
- `feedback_remove_verification_logs` — commit 前 LL_INFOS hook 除去
- `project_ayastorm_r41_vulkan_migration` — Phase 1.B PB-4.4 完了

---

## §6 次 session 着手 1 line

**「前 session で Phase 1.B PB-4.4 integer index 経路 uniform2f Vulkan path 分岐追加 完了 (commit `71576ff1e4`、+15 line)。本 session = **AYA 判断不要 自走** = **PB-4.5 単独着手** = `uniform3f(U32 index, GLfloat x, GLfloat y, GLfloat z)` (llglslshader.cpp:2354) に同 pattern Vulkan path 分岐追加 (= scalar 引数 → 一時 array `GLfloat tmp[3] = {x, y, z};` + `forwardToUboUpload(loc, tmp, sizeof(tmp))`)、GATE-B 整合 + MUSEUBO-A 整合、06a §5.2 / §5.3 literal、1 method 1 sub-step strict 線形 (= `feedback_ubo_migration_one_at_a_time` 復活、batch 3 緩和終了) → 残 PB-4.6〜.17 → PB-5.1〜.13 → PB-7 → PB-N strict 線形進行。」**
