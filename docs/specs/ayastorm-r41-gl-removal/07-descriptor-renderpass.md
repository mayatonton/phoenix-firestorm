...
|---|---|---|---|
| **7.1** | VMA + descriptor pool sizing 基盤 | llvkloader.{cpp,h} 拡張 (VMA allocator init + descriptor pool 作成 + budget API `VK_EXT_memory_budget` 統合) + 05 §3.6 pool sizing 戦略反映 (.maxSets ~200) | VMA allocator init 成功 + descriptor pool 作成成功 + VRAM budget query 成功 + validation 0 件 |
