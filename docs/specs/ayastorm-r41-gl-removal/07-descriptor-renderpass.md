...
**status**: **active 2026-06-01 re-author (sub-step 4.3-γ'-port-β-prep で sub-doc 06 (α) LL_VULKAN_GLSL macro switch 採用と同期、§3.1 sub-step 7.2-7.4 binding 番号 全 base file 共通既定値確定、AYA review 待ち / 旧 closed 2026-05-29 Pattern α 一括 draft + AYA review PASS は §6.5 改訂履歴で歴史保存)**
...
| **7.2** | set=0 per-frame 配線 (**binding 番号確定 2026-06-01**: binding 0 = ViewProj UBO / binding 1 = Lights UBO / binding 2 = Atmosphere UBO、shadow map sampler ×4 / env cubemap sampler ×4 / atmospherics LUT / windlight LUT / AYAstorm picker output buffer は後続 binding に配置) | `VkDescriptorSetLayout` (set=0) 作成 + camera/light matrix UBO + shadow map sampler ×4 + env cubemap sampler ×4 + atmospherics LUT + windlight LUT + AYAstorm picker output buffer (~30 sampler 集約) + 段階 3 sub-step 3.3 matrix stack → push constant 化と同期 + **領域 6 (α) bundle-A uniform → UBO 化との binding 番号同期** (sub-doc 06 §1.2.4 全 base file 共通既定値遵守) | viewer 起動時 set=0 layout 作成成功 + 1 frame 内 `vkUpdateDescriptorSets` + `vkCmdBindDescriptorSets` 動作 + 領域 6 sub-step 6.3 bundle-A patch 完遂後 shader 側 `#ifdef LL_VULKAN_GLSL layout(set=0, binding=0) uniform PerFrame { ... }` qualifier 整合 + validation descriptor binding mismatch 0 件 |
| **7.3** | set=1 per-material 配線 + material cache (**binding 番号確定 2026-06-01**: binding 0 = MaterialUBO / binding 1 = DiffuseTex / binding 2 = NormalTex / binding 3 = SpecTex、AO/emissive は後続 binding) | `VkDescriptorSetLayout` (set=1) 作成 + diffuse/normal/specular/AO/emissive × material 種別 + material params UBO + material cache (set=1 bind 切替のみで material 切替可) + **領域 6 (α) bundle-A/B patch shader 側 binding 番号と同期** (sub-doc 06 §1.2.4 全 base file 共通既定値遵守) | 12 pool (段階 2 完遂済) で material 種別ごと set=1 bind 切替動作 + 領域 6 sub-step 6.3 bundle-A/B patch 完遂後 shader 側 `layout(set=1, binding=N)` 整合 + material cache hit rate ≥ 80% (validation 計測) (※段階 3 sub-step 3.4-γ で layout + 1 set transit smoke を前倒し、03 §3.1.4 参照、本 sub-step 7.3 着手時は material cache 本実装 [~50 material × frame in flight 3 = 150 pool sizing + cache hit rate ≥ 80%] が残作業) |
| **7.4** | set=2 per-draw + push descriptor 配線 (**binding 番号確定 2026-06-01**: binding 0 = per-draw UBO / binding 1+ = per-draw texture、push_constant 64 byte = mat4 modelMatrix VK_SHADER_STAGE_VERTEX_BIT) | `VK_KHR_push_descriptor` extension enable + set=2 layout (binding ≤ 32) + `vkCmdPushDescriptorSetKHR` 配線 + per-object UBO + per-draw texture (avatar BoM / attachment ~106) + model matrix は push constant 64 bytes (段階 3 sub-step 3.3 と同期) + **領域 6 (α) bundle-A/B patch shader 側 binding + push_constant 受領と同期** (sub-doc 06 §1.2.4 全 base file 共通既定値遵守) | 段階 3 sub-step 3.4 12 pool hook body 配線時に set=2 push descriptor 動作 + push constant 64 bytes mat4 modelMatrix 受領動作 + 領域 6 sub-step 6.3 bundle-A/B patch 完遂後 shader 側 `layout(set=2, binding=N)` + `layout(push_constant)` qualifier 整合 + validation push descriptor range violation 0 件 (※段階 3 sub-step 3.4-δ で avatar SSBO 基本配線時に `vkCmdPushDescriptorSetKHR` 基本配線を前倒し、03 §3.1.4 参照、本 sub-step 7.4 着手時は per-object UBO + per-draw texture binding ≤ 32 全配線が残作業) |
...
| 日付 | 状態 | 経緯 |
|---|---|---|
| 2026-05-29 | closed (Pattern α 一括 draft、AYA review PASS) | descriptor set 3 階層 + 7 pass chain + dynamic rendering 設計確定、領域 7 着手準備 ready |
| 2026-06-01 | active re-author | sub-step 4.3-γ'-port-β-prep で sub-doc 06 (α) LL_VULKAN_GLSL macro switch 採用 (case ② case-validity 部分 falsify 反映、handoff `handoff-substep-4-3-gamma-prime-port-beta-prep.md` commit `485ec3c1be`) と同期、§3.1 sub-step 7.2-7.4 binding 番号 **全 base file 共通既定値確定** (set=0 binding 0=ViewProj / 1=Lights / 2=Atmosphere / set=1 binding 0=MaterialUBO / 1=DiffuseTex / 2=NormalTex / 3=SpecTex / set=2 binding 0=per-draw UBO / 1+=per-draw texture / push_constant 64 byte mat4 modelMatrix VK_SHADER_STAGE_VERTEX_BIT)、領域 6 sub-step 6.3 bundle-A/B patch 完遂後の shader 側 `#ifdef LL_VULKAN_GLSL layout(set=N, binding=M)` qualifier 整合経路確立、sub-doc 06 §1.2.4 全 base file 共通既定値遵守と同期、charter §7.5 boundary refine 履歴と同期 |
