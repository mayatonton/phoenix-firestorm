...
| **領域 7 (descriptor / render pass、active 継続)** | sub-step 7.3 (material cache 本実装) / 7.4 (push descriptor 全配線) / 7.5 (実 attachment 配線) は段階 4-5 並走、frame context 経由 frame state 受渡しと協調 (set=0 PerFrame UBO 更新が frame context 経由)。**η-28 後続 (charter §5.3a)**: descriptor set 帯構成は `design/03-descriptor-set-layout.md` (set=0/1/1a/1b/2/3 4 帯 → 5 帯) + `design/04-ubo-blueprint-84.md` (85 UBO blueprint = set=0:3 / set=1:2 / set=2:26 / set=3:54) source of truth、本 sub-doc 04 frame context は set=0 PerFrame UBO 配置経路 = `design/06a-cache-structure-and-setter-redirect.md` + `design/06c-runtime-binding.md` 整合明示 |
