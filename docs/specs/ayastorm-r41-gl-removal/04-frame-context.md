...
- **scope refine 2026-06-03 (η-28 後続)**: sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-28 で **UBO化への pivot** 確定 (charter §2 領域 4 「scope refine 2026-06-03 (η-28 後続)」 + charter §5.3a 反映)、本 sub-doc 04 段階 4 scope は **UBO 受け入れ準備内包** に拡張 = LLPipelineFrameContext struct field を UBO binding (set=0 PerFrame UBO) 配置 + 12 件 RAII setter dead-store 化と並走で setter family 30 entry point (17 integer + 13 LLStaticHashedString) の UBO redirect 配線 + canary cvar 規約 (η-28 chapter 09 §5.3.1) 反映。詳細設計は `design/01-10` chapter 群 (charter §5.3a) + `design/06a/06a-prep/06b/06c` source of truth、本 sub-doc 04 §3 struct 設計 + §4 setter dead-store 化計画は η-28 後続フェーズ実装入口で UBO 反映 refine 予定
...
| **領域 7 (descriptor / render pass、active 継続)** | sub-step 7.3 (material cache 本実装) / 7.4 (push descriptor 全配線) / 7.5 (実 attachment 配線) は段階 4-5 並走、frame context 経由 frame state 受渡しと協調 (set=0 PerFrame UBO 更新が frame context 経由)。**η-28 後続 (charter §5.3a)**: descriptor set 帯構成は `design/03-descriptor-set-layout.md` (set=0/1/1a/1b/2/3 4 帯 → 5 帯) + `design/04-ubo-blueprint-84.md` (84 UBO blueprint = set=0:3 / set=1:2 / set=2:25 or 26 / set=3:54) source of truth、本 sub-doc 04 frame context は set=0 PerFrame UBO 配置経路 = `design/06a-cache-structure-and-setter-redirect.md` + `design/06c-runtime-binding.md` 整合明示 |
...
| **#5-段階 4 (frame context 経由 descriptor 更新)** | sub-step 4.1 で frame context lifecycle 配線 + set=0 PerFrame UBO 更新が frame context 経由動作、領域 7 sub-step 7.3/7.4/7.5 と協調。**η-28 後続 (charter §5.3a)**: set=0 PerFrame UBO の具体 layout = `design/04-ubo-blueprint-84.md` set=0:3 UBO 確定 (= PerFrame / Projection / Camera 等) + `design/06c-runtime-binding.md` runtime binding 配線 + `design/09-phase-roadmap.md` Phase 0-K migration roadmap source of truth、本 sub-doc 04 §3 struct 設計と η-28 UBO blueprint の field-level mapping は実装入口で refine。**完了 marker**: PerFrame UBO 更新の frame context 経由 access + descriptor set 4 帯 (set=0/1/2/3) → 5 帯 (set=0/1/1a/1b/2/3) 動作維持 + dynamic rendering 動作維持 + canary cvar (η-28 chapter 09 §5.3.1) で UBO redirect on/off 切替動作 | 段階 4 内 satisfy + 領域 7 並走 |
