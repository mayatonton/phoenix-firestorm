...
| **3.3-C-β-2** (実装完了 2026-05-29、AYA launch verify 待ち) | dynamic rendering helper body 実装 (`vkCmdBeginRendering` / `vkCmdEndRendering` = Vulkan 1.3 core via volk function pointer + placeholder attachment による transit smoke + Vulkan 1.3 `dynamicRendering` feature query + enable chain via `VkPhysicalDeviceDynamicRenderingFeatures` pNext) | llvkloader.cpp (+118 行) | ✓ helper body 実装 (`sInDynamicRendering` 静的 flag で対称 pair 保証 + 全 view null no-op path + δ-2 case P gating 継承) + `VkPhysicalDeviceDynamicRenderingFeatures` query + enable chain (`createDevice` 内 `vkGetPhysicalDeviceFeatures2` → `dr_features_enable.dynamicRendering = VK_TRUE` → `device_info.pNext` chain) + INFO marker `"Vulkan 1.3 dynamicRendering feature enabled"` (init 時 1 回) + INFO marker `"beginDynamicRendering : dynamic rendering helper path active"` (in-frame 1 回 one-shot) + `beginFrame()` 内 transit smoke 呼出 (`beginDynamicRendering(0, 0, nullptr, 0, nullptr); endDynamicRendering();`) + `make llrender` build PASS (gcc 13.3.0 / no warning / `libllrender.a` link 成功) / AYA launch verify 待ち (期待: 2 marker 出力 + 既存 8 marker 維持 + Vulkan 系 WARN/ERR 0 件) |
