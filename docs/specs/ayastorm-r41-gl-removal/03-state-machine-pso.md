...
| **3.3-C-δ** (実装完了 2026-05-29、AYA launch verify 待ち) | `LLRenderTarget::flush()` Vulkan path 並走、`endDynamicRendering()` 呼出 + 全 marker 維持 + verify | llrendertarget.cpp (+25 行) | ✓ `flush()` 内 `gGL.flush()` 直後 + `mGenerateMipMaps` の前 + `mPreviousRT` 再帰の前に Vulkan path block 追加 (begin/end pair の 1:1 整合性保証: 前 RT の bindTarget が次の begin を発行する前に現 RT の end を完了) + one-shot marker `"flush dynamic rendering end path active"` + `LLVKLoader::endDynamicRendering()` 呼出 (helper 内 `sInDynamicRendering` guard で γ placeholder begin 時は no-op、領域 7 sub-step 7.5 実 attachment 配線後に始めて `vkCmdEndRendering` 発火) + `make llrender` build PASS (gcc 13.3.0 / no warning) / AYA launch verify 待ち (期待: δ marker 1 件 one-shot + 既存 11 marker 維持 [α 8 + C-β-2 2 + C-γ 1] + Vulkan 系 WARN/ERR 0 件) |
