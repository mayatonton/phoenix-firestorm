...
| **3.3-β-2** | β-1 layout base で `VkBuffer` (HOST_VISIBLE_COHERENT、frame in flight 3 個 × 448 B = ~1.4 KB) + `VkDescriptorPool` + `VkDescriptorSet` allocation + persistent mapping + 初期 zero write smoke (validation 0 件) | llvkloader.cpp | UBO 作成成功 + descriptor set 0 allocation 成功 + validation 0 件 |
