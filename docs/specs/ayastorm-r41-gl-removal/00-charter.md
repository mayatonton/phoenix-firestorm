...
### §5.3a 設計 chapter 群 確定 (sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-28 UBO 全体設計 成果物、2026-06-03 起案完了)

η-28 で本 charter §2 領域 4 拡張 (= UBO化への pivot) を 12 chapter 群で設計確定。本 §5.3a は領域 4 source of truth pointer。
...
| `design/06b-shader-codegen-pipeline.md` | §2 領域 6 (GLSL → SPIR-V build-time codegen + perfect hash) |
| `design/06c-runtime-binding.md` | §2 領域 4 + 7 (runtime UBO binding 配線 + descriptor set 更新経路) |
| `design/07-vulkan-api-state.md` | §2 領域 4 + 7 (Vulkan API state ↔ UBO 帯整合) |
| `design/08-build-codegen-pipeline.md` | §2 領域 6 + 8 (build-time codegen pipeline 配置 + autobuild 統合) |
| `design/09-phase-roadmap.md` | §2 領域 4 (Phase 0-K migration roadmap = LLPipelineFrameContext UBO 化 phased rollout) + 本 charter §4 timeline 整合 |
| `design/10-open-questions.md` | §5.5 (AYA 判断仰ぎ 21 件集約、η-28 期間中に集約された未確定項目) |
| `handoff/archive/handoff-substep-4-3-gamma-prime-port-beta-2-bundle-B-B-question-eta-28-design.md` (handoff §4) | sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-28 全 chapter 起案 → 第二次査読 完遂 handoff (本 charter §2 領域 4 pivot 確定 source) |

= η-28 全 chapter 設計は本 r41 charter §2 領域 4 の **UBO化への pivot** 拡張内容を確定したもので、領域 4 実装 phase 入口で source of truth として参照する (本 charter §2 領域 4 「scope refine 2026-06-03 (η-28 後続)」と相互参照)。
...
- **boundary refine 履歴** (AYA review PASS 反映、sub-doc 内処理可境界の明示記録):
  - **2026-05-31**: 段階 5 LLVertexBuffer Vk 化を sub-step 4.3 内へ前出し (`handoff/archive/handoff-substep-4-3-beta-prep.md` §5、AYA 「OK」承認、sub-step 4.3-β' 完遂 commit `78820a6edf` で literal satisfy)
  - **2026-05-31**: 領域 6 sub-step 6.1 着手 model を **case ② = LLShaderMgr Vulkan path 配線 + glslang library runtime API + SPIR-V cache layer** に refine (`handoff/archive/handoff-substep-4-3-gamma-prime-prep.md` §4、案 A target extension build-time pre-compile から path 変更、根拠 = LL/FS shader variant 爆発 model + LLShaderMgr 既存 preprocessing 1 source of truth + 3 OS 整合)、本 §3 #4 acceptance test procedure + §2 領域 6 risk 内訳 を同時 refine、AYA review 待ち
  - **2026-06-01**: 領域 6 sub-step 6.3 scope を **(α) base 228 file 全件 LL_VULKAN_GLSL macro switch structural rewrite** に refine (`handoff/archive/handoff-substep-4-3-gamma-prime-port-beta-prep.md` commit `485ec3c1be` §1-§2、case ② case-validity 部分 falsify = sub-step 4.3-γ'-port-β-1 実機計測で LLShaderMgr hook fire 517 件全件 / SPIR-V 生成成功率 0% / glslang parse 失敗 3 issue type [non-opaque uniforms outside block + location qualifier 欠如 + entry point missing] 確定 → 旧 A 195 / B 53 分類は build-time pre-compile model 前提のみ成立、case ② runtime model では base 228 file 全件 B 相当)、新 plan = bundle-A (uniform → UBO 化 195 file) + bundle-B (location qualifier 追加 228 file) + bundle-C (entry point + #version 460 + extension declare 228 file) 3 bundle 並列 patch、`#ifdef LL_VULKAN_GLSL` macro switch で GL/Vulkan 並走 (GL path 既存維持 = 本 §3 #1 acceptance 担保)、推定工期 ~1 month (Agent 並列で短縮可)、binding 番号 全 base file 共通既定値確定 (sub-doc 06 §1.2.4 + sub-doc 07 §3.1 sub-step 7.2-7.4 同期: set=0 binding 0=ViewProj / 1=Lights / 2=Atmosphere / set=1 binding 0=MaterialUBO / 1=DiffuseTex / 2=NormalTex / 3=SpecTex / set=2 binding 0=per-draw UBO / 1+=per-draw texture / push_constant 64 byte mat4 modelMatrix)、AYAstorm 改変 11 file untouched 維持 (本 §3 #4 acceptance regression 担保)、sub-doc 06 + sub-doc 07 同期改訂、AYA review 待ち
