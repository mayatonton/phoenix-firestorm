...
**要点**: light cvar 3 UBO (= PointLightF + SpotLightF + PointLightV)、本 UBO `sun_wash`(dead)/`falloff`/`global_light_strength` (offset=16/20/24) SpotLightF と同 cvar 由来 double-write、`viewport` (offset=0) screen-space、per-light `falloff` PerDraw 降格候補 [要 L0-4 結果反映]、`viewport` shader 使用箇所 grep 未取得 [要 verify]、dead `sun_wash` layout 維持必須、工数 L (group 全体)
**関連**: L0-1 dispatch (= point light program 識別) / L0-4 cadence 再評価 (= per-light PerDraw 降格) / SpotLightF (= 同 cvar double-write) / §3.5.16 MultiLight (= multi-spot 経路分岐) / §5.4 visual regression policy
