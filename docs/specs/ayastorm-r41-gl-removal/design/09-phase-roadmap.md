...
**default 提案 = A (R1 不採用、C++17 維持)**: chapter 04 §6.4.7 既述根拠 = R3 name-based dispatch + perfect hash (CHD) + frozen-table 経路で十分高速、R1 効果差 (= compile-time vs runtime 1 indirection) は限定的、C++20 切替 cost (= 3 OS toolchain 確認 + dependent module re-validation + autobuild manifest 変更) との trade-off で A 採用が妥当。R1 は **task 完了後の polish 候補** (= Phase K+4 以降の optimization phase 候補) として保留可。

**確定タイミング**: Phase 1.A 入口で R3 動作確認後再評価可、default A 採用継続でも Phase 1.A 着手可 (= 着手 ready state に影響なし)。AYA 判断本体は §14.5 row 3-14 (Phase 1.A 入口) 保留継続。chapter 10 §1.0 row 29 + §1.3 表 (Q-NTTP) 行で集約 index 化済 (= 2026-06-03 ST-7 sub-task 4 batch で登録)。
...
| 3-11 | autobuild manifest で glslang / spirv-cross / Python version pin 状態確認 (= §5.4.1.5 format pin 用意) **✅ 2026-06-03 ST-7 sub-task 6** (= **設計 phase scope では充足 / 実 pin 追加は Phase 1.A 内 task として確定済** = (a) `autobuild.xml` 現状 grep verify pass = `glslang` / `spirv-cross` / `Python` (case-insensitive) entry 0 件 = 未追加状態確認済 (= 4249 行 manifest 内に既存 `SDL2` / `gstreamer10` 等 dependency と同形式の entry 不在) / (b) chapter 08 §5.4.1.5 line 703 「**format version pin**: autobuild manifest `autobuild.xml` で glslang / spirv-cross version を AYAstorm 既定 (= 現状 1.3.275) に pin、勝手な upgrade を抑制 (= chapter 09 Phase K+4 OpenGL 撤廃時に最終 version 確定推奨)」で pin **仕様 + 意図 + tag 値 + timing** 明示済 (= chapter 08 §5.4.1.1 line 538 も「glslang version 1.3.224+ で stable + AYAstorm autobuild bundle version 1.3.275 以降」記述で version 着地点根拠) / (c) chapter 08 §11.5.1 line 1182-1184 で cache key environment block 内に期待 version 例示済 = `python_version: "3.11.5"` / `glslang_version: "1.3.275.0"` / `spirv_cross_version: "2023-12-07"` (= Codegen tool 側 environment 認識 + cache invalidation key 構成材料 = Phase 1.A 実装 source of truth) / (d) chapter 08 §11.5.2 cache invalidation trigger 表 line 1321「glslang version upgrade」も完備 / (e) 実 manifest 編集 (= 3 dependency entry 追加 + Linux/Win/Mac 3 platform 配信 URL pin) は **Phase 1.A 入口で `feedback_design_phase_no_code_write` 解除 (= row 3-12) と同タイミングで実施予定** (= 設計 phase 中の autobuild.xml 編集は scope 外、本 row 3-11 は「pin 仕様確定 + 実装予定 task 化」を以て ✅、実 pin 追加 commit は Phase 1.A handoff doc (= row 3-13) の Exit Criteria に含む形)) | grep verify pass (= autobuild.xml 現状 entry 0 件 + chapter 08 §5.4.1.5 + §11.5.1 + §11.5.2 pin 仕様完備) | (Phase 1.A 内 task) |
...
| 3-13 | Phase 1.A handoff doc 起案 (= 本 chapter §4 Phase 1.A scope + Exit Criteria 反映) **✅ 2026-06-03 ST-7 sub-task 7** (= `docs/specs/ayastorm-r41-gl-removal/handoff/handoff-substep-4-3-gamma-prime-port-beta-2-bundle-B-B-eta-30-phase1-a-entry.md` 起案完了 = §0 state summary + §1 pre-req 最小読み 3 件 + §1.2 pinpoint Read 8 file + §2 Phase 1.A scope (= 09 §4.1) + Exit Criteria (= 09 §4.2) literal 継承 + §3 sub-task PA-1 〜 PA-8 構成表 (= PA-1 autobuild pin / PA-2 Python base / PA-3 mini-parser / PA-4 std140 + reflection / PA-5 perfect hash / PA-6 cache / PA-7 CMake / PA-8 85 UBO 実行 + Exit 充足検証、strict 線形順序、PA-3 + PA-4 のみ並列可、(Q-NTTP) B/C 採用時 PA-0 = C++20 切替 追加可能性明示) + §4 紐付け持越項目 (= 08 §17 (A1)(P)(G/B3)(B1)(B2)(B4)(B5) 7 件 + autobuild pin 1 件 + Phase 1.B/1.C/2 送り出し 3 件) + §5 規律 (= `feedback_design_phase_no_code_write` 解除 timing 明示 + 10 件 feedback 適用) + §6 9 観点 self-verify PASS + §7 引き継ぎ済 memory 17 件 + §8 次 session 着手 1 line) | `docs/specs/ayastorm-r41-gl-removal/handoff/handoff-substep-4-3-gamma-prime-port-beta-2-bundle-B-B-eta-30-phase1-a-entry.md` 物理存在 + Phase 1.A scope + Exit Criteria + sub-task 構成 + 紐付け持越項目 全件反映済 | Phase 1.A 入口直前 task |
