...
| Mesh body / head | Mesh 资产 UUID 已注册到白名单 (§4)。出厂默认白名单已涵盖主流 Mesh body 与 Mesh head。 |
...
出厂默认白名单已包含主流 Mesh body 与 Mesh head 产品,因此大多数用户无需注册即可直接使用 SSS。
...
4. 勾选 **Enabled SSS**。
5. 看自己角色的脸。如果皮肤仍然像塑料,说明该 Mesh head 可能不在默认白名单中 — 右键头部 → **Add to SSS whitelist**。
6. 想要更柔和的外观,把 **Strength** 调到 0.7。如果皮肤显得"没生气",把 **Glow gain** 上调到 0.3 左右。
...
白名单默认值是精选的主流 Mesh body / head UUID 列表,重置会覆盖你的自定义添加 — 请知情后使用。
...
| 脸柔和但手脚仍然塑料感 | 手脚是独立 Mesh 资产、未注册 | 右键对应部件 → Add to SSS whitelist |
