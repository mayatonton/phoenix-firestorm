...
### 10.8 §3.3 cvar split 試行と revert (2026-05-20)

§3.3 「BD/AY default が異なる 32 cvar」の mode 別 tuning 隔離手段として、`{base名}Cinematic` suffix で cvar を物理分離する案を一度 land させた (2026-05-20 commit `0975aa7ec4` + bypass fix `c23ca1d2af`)。AYA hands-on 検証で動作はしたものの、後続の UI binding audit ([`ayastorm-r30-p5-bd-ui-binding-audit-spec.md`](./ayastorm-r30-p5-bd-ui-binding-audit-spec.md)) で以下が判明:

- Firestorm 既存 UI (graphics_advanced / phototools / performance_preferences) の slider/combo は base cvar 名で binding されており、`*Cinematic` 系を読まない。mode 2 で Firestorm 既存 UI を触っても効かない / 既存 UI を生かすには XUI 側にも mode-aware binding 機構が必要
