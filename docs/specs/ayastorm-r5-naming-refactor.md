...
- `docs/specs/spec_positional_stream_audio.md` — 18 occurrences (仕様書)
...
| 同 (1580) | "Parcel owner: enforce these settings on visiting AYAstorm users" ヘッダ | "Parcel owner: enforce Parcel Hide on visiting AYAstorm viewer users" (機能名を明示 + project 名として "AYAstorm viewer" と表記) |
| 同 (1591) | "Add a tag to your parcel description ... When an AYAstorm user is standing in your parcel, the tag overrides their personal settings. ..." | "Add a [parcelhide:...] tag to About Land &gt; Description to enforce Parcel Hide on visiting AYAstorm viewer users. Keys are lowercase. Defaults when present: hideoutside=true, keepavatars=false, keepownobject=false. Legacy [AYAstorm:...] form is also accepted." |
| 同 (1601-1604) | `[AYAstorm:...]` 例示 4件 (各行に inline comment 付き) | `[parcelhide:...]` タグ形 4件のみ (inline comment は説明文側に集約。狭幅パネルでの右切れ回避) |

**実装メモ (XUI レイアウト調整):**

レビュー時に表示が右端で切れる/折り返しが乱れる問題が指摘され、以下も同時に調整した (純粋な命名整理を超える内容なので別コミット `5171b67d3c`):

- `FSAYAParcelTagDesc`: `height` 60 → 80, テキスト本文の XML 行頭インデント (12 スペース) を除去 (XUI が leading whitespace を保持して左にずれて表示される現象の回避)
- `FSAYAParcelTagExamples`: `height` 80 → 64, 同じく行頭インデント除去
- `FSAYAParcelTagHeader`: 行頭インデント除去
- ウィジェット ID の `FSAYA*` プレフィックスは温存 (内部識別子で他から参照されておらず、改名は churn のみで実利益なし)
...
| ファイル | 方針 |
|---|---|
| `docs/specs/spec_positional_stream_audio.md` | **更新**: 仕様書として現行表記に揃える。冒頭に "r5 で `[ayastream:...]` から rename。旧 prefix は恒久 alias" を追記 |
