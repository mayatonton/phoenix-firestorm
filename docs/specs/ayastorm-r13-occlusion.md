...
**仕様**: `docs/specs/spec_obb_occlusion.md`
...
viewer-only の改修。配信側パイプラインは r9 / r10 / r11 / r12 流用、追加変更なし。検証材料は r12 流儀で `docs/archive/r13/` に scene 構築手順 + audio 素材スクリプトを置く。
...
**ファイル**:
- `docs/specs/spec_obb_occlusion.md` (新規、本書 §1 / §2 と同構成、初版)
- `docs/ayastorm-stream3d-roadmap.md` §3 r13 (旧 r13+ basket → 新仕様、r14+ 項目追加、ロードマップ題名 r12 → r13 に拡張)
- `docs/archive/r13/fmod_geometry_survey.md` (新規、P0 調査記録、FMOD geometry API の本 viewer 内利用例ゼロを確認、`indra/llaudio/` の FMOD include 状況、`LLViewerObject::getPositionRegion/getRotationRegion/getScale` 取得経路を確認)
...
**目的**: spec §5 の 4 種 scene (静的壁のみ / 扉付き / linkset 壁 / mesh 壁) を再現可能に構築する手順を `docs/archive/r13/` にドキュメント化。audio 素材は r12 のスクリプト (`docs/archive/r12/gen_upmix_test_material.sh`) を流用。

**ファイル**:
- `docs/archive/r13/build_test_scene.md` (新規、scene A/B/C/D 構築手順、prim サイズ / material flag / LSL door script 含む)
- `docs/archive/r13/door_toggle.lsl` (新規、scene B 用の単純な扉開閉 LSL、`llSetRot` で 90° 回転 toggle)
...
- **O7 (`Stream3DOcclusion = 0` master OFF)**: 実装済 (S32 sentinel、disabled でも smoothing 経由で bypass まで ramp)、live-toggle 体感確認は P10
...
| 設計判断 (旧 r13+ basket 降格 / OBB 単独で shipping / 役割分担導入) | `docs/specs/spec_obb_occlusion.md` §1 / §2.2 / §2.3 |
| リスク R1〜R9 (内容 + 縮退策) | `docs/specs/spec_obb_occlusion.md` §7 |
| 受入条件表 (O1〜O12 + 互換 + 安定性) | `docs/specs/spec_obb_occlusion.md` §6 |
| OBB 抽出 / occlusion 値の決定 / lifecycle / 動的プリム追従 | `docs/specs/spec_obb_occlusion.md` §4.3 / §4.4 / §4.5 |
| debug settings 2 件 (`Stream3DOcclusion` master sentinel + `Stream3DOccluderRange`) | `docs/specs/spec_obb_occlusion.md` §4.7 |
| r11 venue / r12 upmix との直交性 | `docs/specs/spec_obb_occlusion.md` §4.6 |
| r14+ への持ち越し | `docs/specs/spec_obb_occlusion.md` §9 |
| r12 impl record (本書のテンプレート) | `docs/ayastorm-r12-stereo-upmix.md` |
| r12 spec (前提) | `docs/specs/spec_stereo_upmix.md` |
...
| 検証 scene 構築 | `docs/archive/r13/build_test_scene.md` (P9 で作成予定) |
| 検証材料生成 (audio) | `docs/archive/r12/gen_upmix_test_material.sh` (流用) |
