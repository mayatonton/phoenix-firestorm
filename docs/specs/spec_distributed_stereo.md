...
> **前提**: `docs/specs/spec_positional_stream_audio.md` (3D Stream 本体仕様、r5 改訂後)
> **前提 (実装基盤)**: `docs/specs/spec_stream3d_decode_thread.md` (r7 で完了した decode thread 化)
...
- G5. 書式エラー / 構造エラー時はユーザーにわかる形で通知 (書式例つき)、同種エラーは 30 秒間は重複抑制
...
3. 各 prim の description を `parseDistributedStereoTag()` に通し、`ch` を持つものをスピーカーとして収集
...
**同期メカニズム**: 全 channel に同一 `setDelay(start_dspclock, 0, false)` で起動 → サンプル単位の同期 (r7 stereo の実装をそのまま N 個に拡張)。

#### 4.5.1 ring buffer モデル: multi-tail SPSC (frame-based)

```
ring 内部: source frame をインタリーブ格納
  N_track = 1 (mono source):  [s0, s1, s2, ...]
  N_track = 2 (stereo source): [L0, R0, L1, R1, ...]
  N_track = 6 (r10 5.1 source): [FL, C, FR, SL, SR, LFE, FL, ...]  (Vorbis 順)

writer × 1 (decode thread):
  writeIdx_frames を 1 個だけ持つ
  writeAvailable_frames = cap − 1 − max_i(writeIdx − readIdx_i)  ← 最遅 reader に律速

reader × N (speaker, mixer thread の pcmReadCallback):
  各 reader は自分の readIdx_frames を持つ
  ch=L → track[0] を読む
  ch=R → track[1] を読む (mono source なら track[0])
  ch=M → stereo source: (track[0]+track[1]) * 0.5
         mono source:   track[0]
```

**選択理由**: spec §4.5 の代替案 (mixer 前段にスナップショット) と比較し、コピーコストと遅延を最小化できる。最遅 reader による律速は kRingFrames (32k frames ≈ 0.74s @ 44.1kHz) が十分に大きいため、mixer thread の通常のジッタでは問題にならない。万一 reader が starve した場合は writer 側が一時的に書き込みを待つだけで、source の連続デコードが乱れることはない (FMOD source 側の内部バッファが吸収)。

#### 4.5.2 5.1 への布石 (r10 想定)

ring の **track 数 (N_track) は実装定数化せず構築時パラメータ**として渡す:
...
r8 完成時点で `n_tracks` は 1 (mono) または 2 (stereo)、`n_readers` は speakers 数。r10 で 5.1 ソース対応する際は `n_tracks=6` に切り替え、ch enum に `FL/FR/C/LFE/SL/SR` を追加し、ch → track index の mapping table を codec 別に持つだけで済む。詳細は §9.1。
...
| ID | 内容 | 確認方法 |
|---|---|---|
| F1 | 新書式 root + 子 N 個 (N=2,4,8,16) で再生 | 各 N で stereo source 再生、左右移動で減衰確認 |
| F2 | `ch=M` の sum-to-mono が L/R 平均値を出力 | テスト用 L=440Hz / R=880Hz 素材で M スピーカーの周波数解析 |
| F3 | per-speaker `range` 上書きが効く | 子 1 個だけ `range:50`、他 `range:10` で距離減衰の差を観察 |
| F4 | per-speaker `volume` が反映される | 子 4 個に volume 1.0 / 0.8 / 0.5 / 0.2 を設定し聴感確認 |
| F5 | speakers 17 個書いた場合に 16 個まで採用 + 警告通知 | 通知文言と再生本数を確認 |
| F6 | 子プリム description 後付けで反映 (LSL `llSetObjectDesc`) | 起動後に `ch:L` を子に書き込み、数秒以内に再評価 |
| F7 | リンク (link) / アンリンク (unlink) で binding 再評価 | 動的に link 操作してスピーカー追加/削除を確認 |
| F8 | 書式エラー時の通知に書式例が含まれる | `{ch:X}` (不正値) を書いて通知文確認 |
| F9 | 同一 prim 同種エラーは 30 秒抑制される | 同じ不正タグを連打して 1 件/30 秒に絞られることを確認 |
...
`Stream3DRolloffMin` は本書式では参照されない (内部固定 1.0m)。mono タグ (`[3dstream:...]`) の `parseTag` パスでは引き続き使う。
...
- r8 はステレオ系タグの「**置き換え**」リリース: 旧書式 `[3dstream-stereo:{l:N}{r:N}{min:N}{max:N}]` (r5〜r7) は r8 で**廃止**。AYA 個人での内部利用に限定されており、外部公開していなかったため互換維持を見送る判断 (リリース note で告知)。
- 新書式 `[3dstream-stereo:{url:...}{ch:L|R|M}{range:N}{volume:N}]` のみをサポート
- prefix alias `[ayastream-stereo:...]` は新書式の代替名として継続サポート (r5 の rename 経緯)
- mono タグ `[3dstream:...]` は r8 では一切触らない (本書式とは独立)

### 7.2 ABI / 公開ヘッダ

`indra/llaudio/llpositionalstream*.{h,cpp}` の mono 経路 (`LLPositionalStream`) は不変。新規:
- `indra/llaudio/llpositionalstreammulti.{h,cpp}` (新規クラス `LLPositionalStreamMulti`、F3 で導入)
- `indra/llaudio/llmultitailring.{h,cpp}` (新規クラス `LLMultiTailRing`、F3-1 で導入。1 writer × N readers × N_track inter-leaved ring)

既存の `LLPositionalStreamStereo` (r5〜r7) は debug toggle (`Stream3DDebugStereoPlay`) で残留するが、prim 経由の binding からは外す。F3 完了後に debug toggle ごと削除するか判断する。

manager (`llpositionalstreammgr.{h,cpp}`):
- 旧書式関連を**削除**: `StereoTagData` / `parseStereoTag` / `StereoBinding` / `mStereoBindings` / `evaluateStereoBinding`
- 新規追加: `DistStereoTagData` / `parseDistributedStereoTag` / `DistributedStereoBinding` / `mDistributedBindings` / `evaluateLinkset` (F2 以降)
- mono 経路 (`TagData` / `parseTag` / `Binding` / `mBindings` / `evaluateMonoBinding`) は不変
...
| R5 | 通知の重複抑制が破綻 (key 設計ミスで抑制効かない / 抑制しすぎる) | ErrorThrottleKey は `(prim_id, error_kind)` で固定、F4 でテスト |
| R6 | sum-to-mono の clipping (L+R が 0dB 超え) | 0.5f 倍で正規化、源音が 0dB に達していなければ問題なし。clipping 検出ログを decode thread に追加 |
| R7 | 16 spk 時の FMOD::Channel 数枯渇 | `Stream3DMaxConcurrent` 4 並走 × 16 spk = 64 channel 消費。FMOD 既定 channel 数 (512) には収まる範囲だが、F6 で実測 |
| R8 | r7 で確立した shutdown 不変条件 (decode thread join → FMOD release) を壊す | F3 で `LLPositionalStreamMulti` を `LLPositionalStreamStereo` のテンプレートとして実装、stop() / デストラクタ順序を踏襲 |
...
| 既存 `parseTag` (mono) | `indra/newview/llpositionalstreammgr.cpp` |
| 新規 `parseDistributedStereoTag` | `indra/newview/llpositionalstreammgr.cpp` (F1 で追加) |
