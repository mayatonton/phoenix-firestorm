...
> **前提**: `docs/specs/spec_stereo_upmix.md` (r12 stereo→5.1 upmix)
...
- `docs/specs/spec_5_1ch_placement.md` (r10) で確立した per-channel placement の経路は本書でも維持。occlusion は placement の出力に対する FMOD `set3DOcclusion` 自動適用として乗る (= placement 側に変更ゼロ)
- `docs/specs/spec_binaural_venue_reverb.md` (r11) の venue reverb との関係は §4.6 で明記 (occlusion の `reverbOcclusion` が r11 ChannelGroup 末尾の reverb send をゲートする)
- `docs/specs/spec_stereo_upmix.md` (r12) の upmix 経路にも変更なし (per-speaker channel の位置は r10 placement で決まり、occlusion はその位置を入力にするだけ)
...
`docs/archive/r13/build_test_scene.md` (新規) として scene 構築手順をドキュメント化。最小構成:
...
audio 側は r12 の検証材料 (`docs/archive/r12/gen_upmix_test_material.sh`) を流用:
...
- **P10**: 検証 scene 構築手順ドキュメント化 (`docs/archive/r13/build_test_scene.md`) + 検証実行 (O1〜O12)
