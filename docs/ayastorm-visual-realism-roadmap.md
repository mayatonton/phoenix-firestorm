...
### r18: 雲の体積化 + 色温度連動
- ねらい: 既存 flat texture cloud に体積感を付与 + r17 Kelvin modulator を cloud にも連動、写真撮るに値する空の核
- スコープ: (A) cloud shader の volumetric 化 (analytic + 軽量 raymarch、heavy raymarch 不採用) + (B) r17 helper `LLSettingsVOSky::getR17SunModulator` を CLOUD_COLOR uniform push 直前にも適用、sky / scene / cloud の 3 経路で時間帯色温度を完全整合
- 含まない: 雲影 (地表に雲の縞模様) は r19+ 候補に降格 (scope 膨張回避)
- 工数感: 3〜4 日 (3 OS 込み、未確定)
- spec: `docs/ayastorm-r18-cloud-volumetric.md`
...
| r17 時間帯色温度 | 2〜3 日 | **実装完了** (P1.a + KNOWN_SKY_LEGACY_MIDDAY UUID gate、Linux 体感 PASS、3 OS ビルド / tag は r13+r14+r15+r16+r17+r18 一括で実施予定) |
| r18 雲の体積化 + 色温度連動 | 3〜4 日 | **着手** (P0 Survey 未着手、spec 起票済 `docs/ayastorm-r18-cloud-volumetric.md`、scope = 体積化 + r17 helper の cloud_color 連動、cloud shadow は r19+ に分離) |
