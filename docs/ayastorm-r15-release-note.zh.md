...
用于粘贴到 GitHub release 页面的短文。**r15 是继 r12.1 (即 r10→r12.1 跳跃后首个公开 release) 之后的下一个公开 release，把 r13 + r14 + r15 三步以一次跳跃合并发布** — 同时携带音响表现章的最后一根支柱 (标签驱动遮蔽) 与新章「视觉真实感」的第 1+2 弹 (空气体积感 + 光线穿空)。

功能细节常驻于用户指南 (`doc/3dstream-tag-guide.{ja,en,zh}.md`) 与规格 / 路线图。本公告仅做链接 + 差分要点。
