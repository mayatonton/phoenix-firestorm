...
layout(push_constant) uniform BumpF_AlphaMaskPC
{
    layout(offset = 64) float minimum_alpha;
};
layout(set = 1, binding = 1) uniform sampler2D diffuseMap;
layout(set = 1, binding = 2) uniform sampler2D bumpMap;
layout(location = 0) in vec3 vary_mat0;
layout(location = 1) in vec3 vary_mat1;
layout(location = 2) in vec3 vary_mat2;
layout(location = 3) in vec4 vertex_color;
layout(location = 4) in vec2 vary_texcoord0;
