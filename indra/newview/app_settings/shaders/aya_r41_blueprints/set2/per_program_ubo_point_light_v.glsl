...
// PerProgramUBO_PointLightV blueprint
// 旧 (Phase 1.A 起案時): set=2 binding=5
// 新 (Phase 2.L0 step 2-batch-0-a 同期書換 = 2026-06-06 phase E、commit f91cda6677 actual class*/ 対応): **set=1 binding=44 (slot 44)**
...
//         旧 set=2 帯 binding=5 → 新 set=1 帯 binding=44 / cadence=PerProgram / 2 member (vec3 + float)
// Note:   spotLightF.glsl:151 で declared-but-unused (= host bind は同 binding 共有、
//         frag は別 UBO 経由で size 参照、η-28-C type 3 範式)。

#version 450

layout(std140, set = 1, binding = 44) uniform PerProgramUBO_PointLightV
{
    vec3  center;
    float size;
};
