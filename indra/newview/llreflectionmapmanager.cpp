...
#include "llframetimer.h"                // <FS:AYAstorm> CPU perf 章 案 O: LLFrameTimer::getFrameCount
#include "llayastormperflog.h"           // <FS:AYAstorm> CPU perf 章 案 O: worker_enqueue / worker_apply zone
...
LLReflectionMapManager::LLReflectionMapManager()
{
    ...
    initCubeFree();
    // <FS:AYAstorm> CPU perf 章 案 O: worker spawn (CPU thread のみ、GL context 不要)
    mOcclusionWorker = std::make_unique<LLReflectionOcclusionWorker>();
    // </FS:AYAstorm>
}

// <FS:AYAstorm> CPU perf 章 案 O: destructor (unique_ptr destruction で worker thread を join)
LLReflectionMapManager::~LLReflectionMapManager()
{
...
{
    // <FS:AYAstorm> CPU perf 章 案 O: worker offload path (cvar gate, default OFF)
    // r40 phase2 case O gate verdict = REJECT (2026-05-26): doOcclusion 内 CPU 占有率 ~1%、
    // GL 99%、worker 剥がし上限 ~6us vs 50% gate 要求 2850us。詳細 phase2-case-O-plan.md §12。
    // path はクラッシュ修正済 (raw ptr non-owning) のため experiment 用に残す、default 0。
    ...
    if (!worker_path)
    {
        // 旧 single-pass: 全 probe を main thread で順次処理 (cvar=0 もしくは worker 未初期化)
        LLVector4a eye;
    ...
    // (1) 前 frame の worker 結果を取得し apply (1 frame lag 非同期式)
    ProbeOcclusionResult result;
    bool got_result = mOcclusionWorker->wait_get_result(result);
    if (got_result)
    {
        AYAPERF_ZONE("worker_apply");
        // 1 frame 前 snapshot 内の raw probe* が現 mProbes に在籍しているかを validate。
        // 在籍しない (probe が destroy された) ものは skip — raw を deref しない。
