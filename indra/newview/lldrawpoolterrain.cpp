...
// <AYAstorm r41> sub-step 3.4-δ-3: renderFull4TU / renderFull2TU / renderSimple
// 3 関数は段階 2 (Core profile 化) で fixed-function texgen path として死蔵化済、
// caller ゼロ確認のうえ GL fixed-function texgen 物理削除目的で関数本体ごと撤去。
// </AYAstorm r41>
...
void LLDrawPoolTerrain::recordPoolDraws(VkCommandBuffer cmd_buf)
{
    ...
    if (!logged_once)
    {
        ...
        LL_INFOS("Vulkan") << "Terrain fixed-function texgen path physically removed (sub-step 3.4-d-3)" << LL_ENDL;
        logged_once = true;
    }
