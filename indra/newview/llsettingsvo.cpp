...
void LLSettingsVOSky::applySpecial(void *ptarget, bool force)
{
    ...
    // <FS:AYA r17> Color Temperature: 太陽 elevation から派生する Kelvin modulator を
    //   sky path (SUNLIGHT_COLOR / AMBIENT) に適用。scene path (LLPipeline::mSunDiffuse)
    //   は pipeline.cpp::setupHWLights で同じ helper を呼んで modulate するため、
    //   sky と scene で同じ色温度が適用される設計。
