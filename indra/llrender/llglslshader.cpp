...
// test UBO bring-up = `Global_ReflectionProbes` (= PC-1 で codegen 確定済 singleton UBO)
// 経路の test driver。本関数は二段構え運用:
//
//   (PC-1 layered safety) `ubo::lookup_block()` + 5 件 llassert で
//     PC-1 確定 contract (block name / set=0 / binding=3 / size=256B / cadence=SINGLETON=5)
//     と block metadata の整合 runtime check。codegen 出力 drift を debug build で即停止
//     検出 (release では silent、codegen unittest 130/130 + 本 runtime assert で 2 層 safety)。
//
//   (PC-6ε-1 本格 flush 経路) PC-2 の `forwardToUboUpload` 経由 dummy 書込 + shell
//     dirty flag を `LLVKLoader::flushSingletonUbos()` 経由置換。6 cadence (= 5 cadence
//     + singleton) 体系成立。bringupTestUBO 呼出毎に singleton cadence flush が ring
//     buffer 経路 (= flushDummyUboWrite helper、256 B allocate + memset 0 + side-table
//     mapped pointer 解決) 通電。
//
// 設計根拠 (= design canonical naming 整合):
//   - design 02 §3 で `Global_` prefix = singleton cadence と明示分類
//     (= 5 cadence prefix + singleton prefix = 6 cadence 体系)。
...
// 100% 維持。mUseUBO=false default で本 hook は実走しない。
// GATE-B 整合: mUseUBO runtime gate 未依存 (= PC-6α..δ + PC-6ε-1 同形、Vulkan
// initialization 層単独で flush 駆動)。
void LLGLSLShader::bringupTestUBO()
{
    ...
    // <AYAstorm r41 PC-6ε-1> singleton cadence flush 駆動位置 (= 6 cadence 体系成立)。
    // PC-2 forwardToUboUpload(loc, s_dummy, ...) + s_test_ubo_dirty shell flag は
    // 本 PC-6ε-1 で flushSingletonUbos() 経由 ring buffer 経路に本格置換。
    // design 02 §3 (Global_ prefix = singleton cadence 明示分類) +
    // design 06c §2.2 (Global_ReflectionProbes singleton 配置例) 整合。
    LLVKLoader::flushSingletonUbos();
    // </AYAstorm r41 PC-6ε-1>
}
...
void LLGLSLShader::uniform3f(U32 index, GLfloat x, GLfloat y, GLfloat z)
{
    ...
    if (mProgramObject)
    {
        ...
        if (mUniform[index] >= 0)
        {
            ...
            if (iter == mValue.end() || shouldChange(iter->second, vec))
            {
                // r41 sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-30 Phase 1.B PB-4.5:
                // integer index 経路 Vulkan path 分岐追加。spec 06a §5.2 / §5.3 literal 準拠。
                // GATE-B = #ifdef LL_VULKAN_GLSL 不使用、mUseUBO runtime flag 単独 gate。
                // MUSEUBO-A = mUseUBO=false default で本 block 走らず既存 OpenGL 挙動 100% 維持。
                // scalar 引数 → 一時 array pattern (= tick-prompt §4 type/size 早見表準拠)。
...
void LLGLSLShader::uniform4f(U32 index, GLfloat x, GLfloat y, GLfloat z, GLfloat w)
{
    ...
    if (mProgramObject)
    {
        ...
        if (mUniform[index] >= 0)
        {
            ...
            if (iter == mValue.end() || shouldChange(iter->second, vec))
            {
                // r41 sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-30 Phase 1.B PB-4.6:
                // integer index 経路 Vulkan path 分岐追加。spec 06a §5.2 / §5.3 literal 準拠。
                // GATE-B = #ifdef LL_VULKAN_GLSL 不使用、mUseUBO runtime flag 単独 gate。
                // MUSEUBO-A = mUseUBO=false default で本 block 走らず既存 OpenGL 挙動 100% 維持。
                // scalar 引数 → 一時 array pattern (= tick-prompt §4 type/size 早見表準拠)。
...
void LLGLSLShader::uniform1fv(U32 index, U32 count, const GLfloat* v)
{
    ...
    if (mProgramObject)
    {
        ...
        if (mUniform[index] >= 0)
        {
            ...
            if (iter == mValue.end() || shouldChange(iter->second, vec) || count != 1)
            {
                // r41 sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-30 Phase 1.B PB-4.9:
                // integer index 経路 Vulkan path 分岐追加。spec 06a §5.2 / §5.3 literal 準拠。
                // GATE-B = #ifdef LL_VULKAN_GLSL 不使用、mUseUBO runtime flag 単独 gate。
                // MUSEUBO-A = mUseUBO=false default で本 block 走らず既存 OpenGL 挙動 100% 維持。
                // ptr+count pattern (= count * sizeof(GLfloat))。
                if (mUseUBO)
                {
                    llassert(index < mUniformUBOLoc.size());
                    const ubo::UniformLocation& loc = mUniformUBOLoc[index];
...
void LLGLSLShader::uniform2fv(U32 index, U32 count, const GLfloat* v)
{
    ...
    if (mProgramObject)
    {
        ...
        if (mUniform[index] >= 0)
        {
            ...
            if (iter == mValue.end() || shouldChange(iter->second, vec) || count != 1)
            {
                // r41 sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-30 Phase 1.B PB-4.10:
                // integer index 経路 Vulkan path 分岐追加。spec 06a §5.2 / §5.3 literal 準拠。
                // GATE-B = #ifdef LL_VULKAN_GLSL 不使用、mUseUBO runtime flag 単独 gate。
                // MUSEUBO-A = mUseUBO=false default で本 block 走らず既存 OpenGL 挙動 100% 維持。
                // ptr+count pattern (= count * 2 * sizeof(GLfloat))。
                if (mUseUBO)
                {
                    llassert(index < mUniformUBOLoc.size());
                    const ubo::UniformLocation& loc = mUniformUBOLoc[index];
...
void LLGLSLShader::uniform3fv(U32 index, U32 count, const GLfloat* v)
{
    ...
    if (mProgramObject)
    {
        ...
        if (mUniform[index] >= 0)
        {
            ...
            if (iter == mValue.end() || shouldChange(iter->second, vec) || count != 1)
            {
                // r41 sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-30 Phase 1.B PB-4.11:
                // integer index 経路 Vulkan path 分岐追加。spec 06a §5.2 / §5.3 literal 準拠。
                // GATE-B = #ifdef LL_VULKAN_GLSL 不使用、mUseUBO runtime flag 単独 gate。
                // MUSEUBO-A = mUseUBO=false default で本 block 走らず既存 OpenGL 挙動 100% 維持。
                // ptr+count pattern (= count * 3 * sizeof(GLfloat))。
                if (mUseUBO)
                {
                    llassert(index < mUniformUBOLoc.size());
                    const ubo::UniformLocation& loc = mUniformUBOLoc[index];
...
void LLGLSLShader::uniform4fv(U32 index, U32 count, const GLfloat* v)
{
    ...
    if (mProgramObject)
    {
        ...
        if (mUniform[index] >= 0)
        {
            ...
            {
                LL_PROFILE_ZONE_SCOPED_CATEGORY_SHADER;
                // r41 sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-30 Phase 1.B PB-4.12:
                // integer index 経路 Vulkan path 分岐追加。spec 06a §5.2 / §5.3 literal 準拠。
                // GATE-B = #ifdef LL_VULKAN_GLSL 不使用、mUseUBO runtime flag 単独 gate。
                // MUSEUBO-A = mUseUBO=false default で本 block 走らず既存 OpenGL 挙動 100% 維持。
                // ptr+count pattern (= count * 4 * sizeof(GLfloat))。
                if (mUseUBO)
                {
                    llassert(index < mUniformUBOLoc.size());
                    const ubo::UniformLocation& loc = mUniformUBOLoc[index];
...
void LLGLSLShader::uniform4uiv(U32 index, U32 count, const GLuint* v)
{
    ...
    if (mProgramObject)
    {
        ...
        if (mUniform[index] >= 0)
        {
            ...
            {
                LL_PROFILE_ZONE_SCOPED_CATEGORY_SHADER;
                // r41 sub-step 4.3-γ'-port-β-2-bundle-B-B?-η-30 Phase 1.B PB-4.13:
                // integer index 経路 Vulkan path 分岐追加。spec 06a §5.2 / §5.3 literal 準拠。
                // GATE-B = #ifdef LL_VULKAN_GLSL 不使用、mUseUBO runtime flag 単独 gate。
                // MUSEUBO-A = mUseUBO=false default で本 block 走らず既存 OpenGL 挙動 100% 維持。
                // ptr+count pattern (= count * 4 * sizeof(GLuint))。
                if (mUseUBO)
                {
                    llassert(index < mUniformUBOLoc.size());
                    const ubo::UniformLocation& loc = mUniformUBOLoc[index];
