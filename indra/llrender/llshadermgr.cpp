    ...
    if (texture_index_channels > 0 && type == GL_FRAGMENT_SHADER)
    {
        ...
        */
        ...
        if (use_bindless_heap)
        {
            ...
            extra_code_text[extra_code_count++] =
                strdup("struct AyaDrawData { uvec4 tex_slots; vec4 spec_color; vec4 misc; float object_alpha; uint normal_slot; uint specular_slot; uint dd_pad; };\nlayout(set=2, binding=0, std430) readonly buffer AyaDrawDataBlock { AyaDrawData aya_dd[]; };\n");
            extra_code_text[extra_code_count++] =
...
// [/RLV:KB]
    ...
    mReservedUniforms.push_back("dither_tex");
    mReservedUniforms.push_back("texture0");
    mReservedUniforms.push_back("texture1");
    mReservedUniforms.push_back("predicationTex");
    mReservedUniforms.push_back("srcMap");
