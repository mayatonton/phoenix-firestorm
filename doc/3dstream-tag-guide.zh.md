...
`hall_medium` / `hall_large` / `cathedral` (长尾会场) 的 CPU 消耗明显高于 `dry` / `room_*`。请按所需会场感选择 — 比如 live house 可用 `hall_small` 或 `club`，不必硬上 `hall_large`。"增量" 列是每个 binding 的开销 (不是每扬声器 × N)。详见 `doc/spec_binaural_venue_reverb.md`。
